---
layout:     post
title:     OKHttp 源码分析（2022.04.09 持续更新中）
...
1. 创建OKHttpClient对象，这个对象是Builder类型的

2. 创建Request对象，这个也是Builder类型的

3. 创建Call对象，通过OKHttpClient类下的newCall方法

   newCall -> RealCall -> newRealCall来创建

4. 创建Response对象，通过Call类中的execute（同步方法）

   execute()最后将request给到Depatcher（分发器）

5. Depatcher，分发器，有线程池，负责将任务分配到running队列或者ready队列，也可以将任务从running队列和ready队列中调出，Depatcher将请求交给Intercepter（拦截器）
